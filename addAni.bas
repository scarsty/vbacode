Sub AnimationOne()
    ...
    Count = 0
    For Each ShapeObject In Application.ActiveWindow.View.Slide.Shapes
        Count = Count + 1
        Set x(Count) = ShapeObject
        'ShapeObject.AnimationSettings.Animate = msoFalse
        'MsgBox (x(Count).Top)
    Next ShapeObject
    'MsgBox (Count)
    For J = Application.ActiveWindow.View.Slide.TimeLine.MainSequence.Count To 1 Step -1
        Application.ActiveWindow.View.Slide.TimeLine.MainSequence(J).Delete
    Next J
    For I = 1 To Count - 1
        For J = I + 1 To Count
